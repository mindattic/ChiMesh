...
**Version 1.0.0** | Chicago LoRa Meshtastic mesh — proof-of-mesh build.
...
```
ChiMesh/
├── README.md                     # this file — build narrative (sections 01-08) AND the landing-page
│                                 #   source, rendered verbatim by MindAttic.Deploy. Its
│                                 #   <!-- CONFIG-WIDGET -->, <!-- PARTS-GALLERY -->, and
│                                 #   <!-- when: key=value -->...<!-- end --> markers are filled in
│                                 #   at render time — do not remove them.
├── ChiMesh.Console.bat           # root-level shortcut -> scripts/cli/ChiMesh.Console.bat
├── package.json                  # Node deps (marked, highlight.js) — installed by `Console update`
├── package-lock.json
├── config/
│   ├── parts.json                # L5 canon-as-data: categories, configAxes, parts (price/specs/tiers), chosen picks
│   ├── versions.json             # pinned upstream version labels (Meshtastic 2.5.x, etc.) — inlined into README.md
│   └── images/                   # 10 per-part photos, base64-inlined into the rendered parts gallery
├── scripts/cli/
│   ├── ChiMesh.Console.ps1       # dispatch-table console: help / update / provision / healthcheck / list-parts / find-deals / pull-latest
│   ├── ChiMesh.Console.bat       # Windows launcher for ChiMesh.Console.ps1
│   ├── provision-node.ps1        # set region/role/channel0/owner on a USB-connected RAK4631, idempotent
│   ├── healthcheck-mesh.ps1      # 6-check smoke test (CLI present, node responds, region, role, channel0, peer count)
│   └── pull-latest-finisher.ps1  # detached helper that finishes pull-latest's file copy after the console process exits
├── tools/
│   └── codex.ps1                 # doctor (validate canon) + digest (regenerate BIBLE.digest.md)
├── docs/
│   ├── BIBLE.md                  # L0 source of truth — architecture, laws, glossary, verified state
│   ├── BIBLE.digest.md           # GENERATED by tools/codex.ps1 digest — never hand-edit
│   ├── AMENDMENTS.md             # L1 append-only change log (CM-A1, CM-A2, ...) — an amendment wins over the bible
│   ├── USER_STORIES.md           # L2 acceptance stories (CM-US-*) — every ✅ cites its verifying test
│   ├── rfc/                      # design notes (e.g. 0001-secured-channel-psk.md)
│   └── data/                     # L5 registration: parts.json + _schema/part.schema.json
└── .claude/                      # Claude Code session config: commands (checkpoint/commit/deploy/do),
                                   #   hooks (inject-digest.ps1, restore-handoff.ps1), statusline.ps1
```

> The landing page (`mindattic.com/chimesh.htm`) is rendered and deployed by the sibling **MindAttic.Deploy** repo — there is no local build or FTP pipeline in this repo. Run `/deploy` to trigger it.

---

## Architecture

ChiMesh splits into a **content half** (this repo) and a **render/deploy half** (sibling repo
`MindAttic.Deploy`), plus standalone **node tooling** that talks to physical hardware over USB.

- This repo owns the build narrative (`README.md`, the file you're reading), the parts catalog
  (`config/parts.json`, `config/versions.json`, `config/images/`), and the PowerShell scripts that
  provision/healthcheck a USB-connected RAK4631 (`scripts/cli/`).
- `MindAttic.Deploy` (sibling, out of this repo) renders `README.md` + `config/parts.json` through
  its `Hardware`-theme catalog template into `out/chimesh.htm` and FTPS-uploads it to
  `mindattic.com/chimesh.htm`. Rendering fills the `<!-- CONFIG-WIDGET -->` picker, the
  `<!-- PARTS-GALLERY -->` cards + running cost total, and toggles `<!-- when: key=value -->` copy
  blocks. Trigger it with `/deploy` — see [`.claude/commands/deploy.md`](.claude/commands/deploy.md).
- Physically, each node is one fixed signal chain: solar panel → TP5000 (LFP-configured) → LiFePO4
  cell → JST-PHR-2 → RAK19003 → RAK4631 → IPEX pigtail → SMA bulkhead → 915 MHz antenna.

See [docs/BIBLE.md §4](docs/BIBLE.md#CM-§4) for the full architecture diagram, component list, and
domain model (nouns/verbs) — this section is a summary, not a duplicate.

---

## Configuration

Parts, pricing, and version facts are canon-as-data — edit these files, not README prose, and the
next `/deploy` picks up the change:

| File | Purpose |
|---|---|
| [`config/parts.json`](config/parts.json) | The parts catalog: `categories` (`core` / `consumable` / `tools`), `configAxes` (`region`, `role`, `deployment`, `antenna` — each becomes a `<select>` in the landing-page picker, matched against this README's `<!-- when: key=value -->` blocks), per-part price/specs/purchase tiers (official/amazon/reputable), and `chosen` (URLs saved by `find-deals`). Registered as L5 canon via [`docs/data/parts.json`](docs/data/parts.json) against [`docs/data/_schema/part.schema.json`](docs/data/_schema); `tools/codex.ps1 doctor` validates every row against that schema and checks `id` uniqueness. |
| [`config/versions.json`](config/versions.json) | Reference snapshot of pinned upstream versions (Meshtastic firmware `2.5.x`, CLI label, board labels, default region/channel, flasher/app URLs). Values are inlined directly into `README.md`; keep this file in sync when upstream ships a new major. |
| [`config/images/`](config/images) | Per-part photos (10 files: `rak4631.png`, `rak19003.png`, `tp5000-lfp.jpg`, etc.), base64-inlined into the rendered parts gallery by `MindAttic.Deploy`'s `src/parts.js`. |

Cite parts from prose as `part.<id>` rather than restating their fields — see
[CM-LAW-7](docs/BIBLE.md#CM-LAW-7) (single home per fact).

---

## Console

Launch the interactive console from the repo root:

```
ChiMesh.Console.bat
```

Or dispatch a command directly — `ChiMesh.Console.bat <command> [args]`:

| Command | What it does |
|---|---|
| `help` | List available commands (also shown with no args, as a looping menu). |
| `update [--clean]` | Install/refresh the Node deps in `package.json` (`marked`, `highlight.js`) into `node_modules/`. `--clean` wipes `node_modules/` + `package-lock.json` first. |
| `provision <node-name> [-Region US] [-Role ROUTER_CLIENT] [-Channel ChiMesh-Test] [-Port COMx]` | Runs `provision-node.ps1` — sets region/role/channel0/owner on the USB-connected node and reads back to confirm. |
| `healthcheck [-Port COMx] [-Verbose]` | Runs `healthcheck-mesh.ps1` — the 6-check smoke test. |
| `list-parts` | Lists the parts catalog grouped by category, with price and any already-chosen purchase URL. |
| `find-deals [core\|consumable\|--all]` | Opens Search/Official/Amazon/Reputable tabs per part; paste back the URL you picked and it's saved to `config/parts.json.chosen`. |
| `pull-latest [--force]` | `git fetch` + stages the latest tracked branch into a temp clone, then hands off to a detached helper (`pull-latest-finisher.ps1`) that overlays it onto the repo after the console process exits (so its own files aren't locked mid-copy). `--force` overwrites even with a dirty working tree. |

**Prerequisites:**
- PowerShell 5.1 (Windows PowerShell — no `pwsh` required)
- `pip install --upgrade meshtastic` (for `provision` and `healthcheck`)
- Node.js + npm (for `update`)
...
There is no application build or automated test suite in this repo. `package.json` declares two
Node deps (`marked`, `highlight.js`) that `ChiMesh.Console update` installs into `node_modules/`,
but nothing under `scripts/cli/` or `tools/` currently requires/imports either package — HTML
rendering now runs entirely in the sibling `MindAttic.Deploy` repo (see
[CM-A1](docs/AMENDMENTS.md#CM-A1)), and these deps predate that move.

The only in-repo automated verifier is the Codex doctor. It must pass before any docs change is
considered done:
...
Regenerate `docs/BIBLE.digest.md` (whenever BIBLE §1/§3/§5/§9 or the latest amendment changes):

```
powershell -NoProfile -ExecutionPolicy Bypass -File tools\codex.ps1 digest
```

`doctor` checks front-matter validity, unique IDs, resolvable cross-references, `config/parts.json`
rows against the L5 schema, that every ✅ story cites an existing-or-best-effort test, cited paths
exist, and digest freshness. Node tooling (`provision`, `healthcheck`) can only be exercised against
physical RAK4631 hardware, so it's unproven in CI — see [docs/BIBLE.md §6](docs/BIBLE.md#CM-§6) for
the honest, per-item verified-state ledger, and [docs/USER_STORIES.md](docs/USER_STORIES.md) for
per-story test citations.
