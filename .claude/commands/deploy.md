...
- `<!-- when: key=value; ... -->` ... `<!-- end -->` -> conditional blocks shown/hidden as the picker changes. Keys/values must match `configAxes` values exactly (e.g. `deployment=rooftop`, `role=client`, `antenna=fiberglass8`). Nested `when` blocks are NOT supported (the regex pairs the first `<!-- end -->`).
