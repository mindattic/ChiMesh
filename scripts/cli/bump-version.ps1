<#
.SYNOPSIS
    Stamp ChiMesh.md with a new revision date and regenerate ChiMesh.htm.

.DESCRIPTION
    The build guide lives at the stable path ChiMesh.md so external links
...
if (-not (Test-Path $mdPath)) { Write-Error "ChiMesh.md not found at $mdPath" }
...
if ($content -notmatch '\*Last updated:\s*(\d{4}\.\d{2}\.\d{2})\*') {
    Write-Error "Could not find '*Last updated: <YYYY.MM.DD>*' line in ChiMesh.md"
}
$oldDate = $Matches[1]

if ($oldDate -eq $newDate) {
    Write-Host "No change - ChiMesh.md is already stamped $newDate."
    exit 0
}
...
Write-Host "OK  rewrote ChiMesh.md"

Write-Host "-> Regenerating ChiMesh.htm"
