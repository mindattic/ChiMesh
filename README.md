...
```
ChiMesh/
├── README.md                  # this file — also the landing-page source rendered by MindAttic.Deploy
├── config/
│   ├── parts.json             # parts catalog (prices, specs, buy links, config axes)
│   └── versions.json          # pinned upstream version labels (Meshtastic 2.5.x, etc.)
├── scripts/cli/
│   ├── ChiMesh.Console.ps1    # interactive console: provision / healthcheck / list-parts / find-deals / pull-latest
│   ├── ChiMesh.Console.bat    # Windows launcher for ChiMesh.Console.ps1
│   ├── provision-node.ps1     # set region, role, channel, owner on a USB-connected RAK4631
│   ├── healthcheck-mesh.ps1   # smoke-test a connected node (region / role / channel / peer count)
│   └── pull-latest-finisher.ps1  # detached helper used by the pull-latest command
├── ChiMesh.Console.bat        # root-level shortcut -> scripts/cli/ChiMesh.Console.bat
├── docs/
│   ├── BIBLE.md               # L0 source of truth (laws, scope, glossary)
│   ├── BIBLE.digest.md        # generated by tools/codex.ps1 digest — do not hand-edit
│   ├── AMENDMENTS.md          # append-only change log
│   ├── USER_STORIES.md        # acceptance stories
│   ├── rfc/                   # design notes
│   └── data/                  # L5 data registration (parts.json schema)
├── tools/
│   └── codex.ps1              # doctor (validate canon) + digest (regenerate BIBLE.digest.md)
└── package.json               # Node deps (marked + highlight.js) used by MindAttic.Deploy render
```
...
ChiMesh.Console.bat list-parts
ChiMesh.Console.bat find-deals core
ChiMesh.Console.bat update
ChiMesh.Console.bat pull-latest
```

**Prerequisites:**
- PowerShell 5.1 (Windows PowerShell — no `pwsh` required)
- `pip install --upgrade meshtastic` (for `provision` and `healthcheck`)
- Node.js + npm (for `update` — installs deps used by MindAttic.Deploy at render time)

---

## Codex tooling

Validate canon (must pass before any docs change is considered done):
