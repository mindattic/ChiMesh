<#
.SYNOPSIS
    ChiMesh console - local builder tasks + remote Pi (chimesh) configuration.

.DESCRIPTION
    Single entry point for both:
      - Local dev: install Node deps, render ChiMesh.htm, bump guide versions,
        FTP-deploy to mindattic.com/chimesh/.
      - Remote Pi: auto-detect "chimesh" on the LAN, then edit its .env
...
.EXAMPLE
    ...
    .\scripts\cli\ChiMesh.Console.ps1 detect                # find the Pi on the LAN
    .\scripts\cli\ChiMesh.Console.ps1 set-model claude-sonnet-4-6
    .\scripts\cli\ChiMesh.Console.ps1 logs
#>
[CmdletBinding()]
param(
...
function Find-Pi {
    $state = Get-State
    $candidates = @($state.host, 'chimesh.local', 'raspberrypi.local') | Select-Object -Unique
    foreach ($name in $candidates) {
        Write-Info "probing $name ..."
        if (Test-PiReachable $name) {
            Write-Ok "found ChiMesh at $name"
            $state.host = $name
            Save-State $state
            return $name
        }
    ...
    Write-Warn2 "tip: find its IP in your router admin page, then: ChiMesh.Console set-host <ip>"
    return $null
}
...
function Cmd-SetModel($a) {
    ...
    Write-Warn2 'restart the service for it to take effect:  ChiMesh.Console restart'
}
...
function Cmd-SetApiKey($a) {
    ...
    Write-Ok 'key updated. Restart:  ChiMesh.Console restart'
}
...
function Cmd-SetTts($a) {
    ...
    # ChiMesh.md Part 08 (Configure chatbot - TTS subsection).
    ...
    if ($val -eq 'elevenlabs') {
        Write-Warn2 'elevenlabs needs the patch in ChiMesh.md section 09 (TTS subsection)'
...
function Cmd-SetLlm($a) {
    # Upstream switches by LLM_SERVER. Default for ChiMesh is anthropic.
...
function Cmd-Deploy($a) {
    # Build ChiMesh.htm fresh and FTP-upload it + ChiMesh.md + index.htm to
...
function Cmd-ApplyDeals {
    ...
    # Single stable file - the date lives inside the content.
    $guide = Get-Item -Path (Join-Path $repoRoot 'ChiMesh.md') -ErrorAction SilentlyContinue
    if (-not $guide) { throw "ChiMesh.md not found in $repoRoot." }
...
function Cmd-PullLatest($a) {
    ...
    # *current* ChiMesh.Console.ps1 file is held open by THIS PowerShell
    ...
    Write-Warn2 '    ChiMesh.Console.bat'
...
function Cmd-SelfUpdate($a) {
    ...
    if (-not $skipModels) {
        ...
        Write-Warn2 '  - ChiMesh.md Part 6 table'
        Write-Warn2 '  - the Pi if a unit is deployed:  ChiMesh.Console set-model <id>'
    }
...
$commands = [ordered]@{
    ...
    'detect'       = @{ Help = 'Find ChiMesh (the Pi) on the LAN. Saves the host for later commands.';                        Action = { Cmd-Detect } }
    'set-host'     = @{ Help = 'Override the Pi hostname/IP. Usage: set-host <name-or-ip> [user]';                            Action = { param($a) Cmd-SetHost $a } }
    'shell'        = @{ Help = 'Open an interactive SSH session to ChiMesh.';                                                 Action = { Cmd-Shell } }
    'status'       = @{ Help = 'Show chatbot.service status on ChiMesh.';                                                     Action = { Cmd-Status } }
    'restart'      = @{ Help = 'Restart chatbot.service on ChiMesh.';                                                         Action = { Cmd-Restart } }
    'logs'         = @{ Help = 'Tail ChiMesh chatbot logs (Ctrl+C to stop).';                                                 Action = { Cmd-Logs } }
    'healthcheck'  = @{ Help = 'Copy scripts/pi/healthcheck.sh to ChiMesh and run it.';                                       Action = { Cmd-Healthcheck } }
    ...
    'deploy'       = @{ Help = 'Build ChiMesh.htm and FTP-upload .md/.htm/index.htm (uses scripts/cli/deploy.settings.json). Add --no-build to skip the rebuild.'; Action = { param($a) Cmd-Deploy $a } }
    'bump'         = @{ Help = 'Stamp ChiMesh.md with today''s revision date (or -To <YYYY.MM.DD>) and rebuild ChiMesh.htm.';  Action = { param($a) Cmd-Bump $a } }
    ...
    'apply-deals'  = @{ Help = 'Stamp chosen URLs into ChiMesh.md.';                                                          Action = { Cmd-ApplyDeals } }
...
function Show-Help {
    ...
    Write-Host "    ChiMesh.Console detect"
    Write-Host "    ChiMesh.Console set-model claude-sonnet-4-6"
    Write-Host "    ChiMesh.Console logs"
    Write-Host ""
}
